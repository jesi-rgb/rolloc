...
/**
 * Directory names to skip entirely when scanning for image files (case
 * -insensitive). These hold data generated by rolloc itself, not original
 * negatives, so re-scanning into them would re-import already-exported
 * positives (and try to invert JPEGs a second time) or the sidecar cache.
 */
const IGNORED_DIR_NAMES = new Set(['exports', '.rolloc-meta']);
...
/**
 * True if any path segment of `relativePath` is an ignored directory (see
 * `IGNORED_DIR_NAMES`). Used to detect and prune stale `Frame` records that
 * were previously (erroneously) created from files inside `exports/` or
 * `.rolloc-meta/` before directory scanning excluded them.
 */
export function isIgnoredPath(relativePath: string): boolean {
	return relativePath.split(/[/\\]/).some((segment) => isIgnoredDir(segment));
}
